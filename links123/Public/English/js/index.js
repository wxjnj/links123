...
//                $("ul.tabs").tabs("div.panes > div.box1");
    //排行榜点击事件
    $('.orderpai').click(function() {
        if ($(".order").position().left < 0) {
            $(".order").animate({'left': '0px'});
            $(".orderpai").css("background", "url(" + PUBLIC + "/English/images/orderpai2.png) no-repeat");
        } else {
            $(".order").animate({'left': '-322px'});
            $(".orderpai").css("background", "url(" + PUBLIC + "/English/images/orderpai1.png) no-repeat");
        }
    })
//                $('.orderpai').toggle(function() {
//                    orderposition = $(".order").position().left;
//                    if (orderposition < 0) {
//                        $(".order").animate({'margin-left': '320px'});
//                        $(".orderpai").css("background", "url(__PUBLIC__/English/images/orderpai2.png) no-repeat");
//                    }
//                }, function() {
//                    $(".order").animate({'margin-left': '0px'});
//                    $(".orderpai").css("background", "url(__PUBLIC__/English/images/orderpai1.png) no-repeat");
//                });
    ...
    $("#J_answerButton").click(function() {
        ...
        if ($(".answer").is(":visible")) {
             if(iframe.size()){ iframe.attr('src', $("#J_media_div").attr('iframesrc')); }
            $(".answer").slideUp("slow");
            ...
            if ($("#J_media_div").attr("media_type") == 1 || $("#J_media_div").attr("media_type") == 2) {
             ...
             if(iframe.size()){
                $("#J_media_div").attr('iframesrc', iframe.attr('src'));
...
function requestQuestion(type) {
    ...
    layer_div("show");
    ajaxRequest = $.post(URL + "/ajax_get_question", {
        voice: voice,
        target: target,
        pattern: pattern,
        object: object,
 ...
 */
function bindLevelUpEvent() {
    $("#J_levelUpButton").click(function() {
        var next_level_li;
        //已是最高级
 ...
 * @author Adam $date2013-07-27$
 */
function bindLevelDownEvent() {
    $("#J_levelDownButton").click(function() {
        var prev_level_li;
