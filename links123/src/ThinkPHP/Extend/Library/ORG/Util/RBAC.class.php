...
class RBAC {
    ...
    //权限认证的过滤器方法
    static public function AccessDecision($groupName=GROUP_NAME) {
        //检查是否需要认证
        if(RBAC::checkAccess()) {
            //存在认证识别号，则进行进一步的访问决策
            $accessGuid   =   md5($groupName.MODULE_NAME.ACTION_NAME);
            if(!isset($_SESSION[C('ADMIN_AUTH_KEY')])) {
                ...
                if(!isset($accessList[strtoupper($groupName)][strtoupper($module)][strtoupper(ACTION_NAME)])) {
