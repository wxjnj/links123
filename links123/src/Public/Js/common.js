...
function showHideSearch() {
    if (document.getElementById('searchM').style.display != 'none') {
...
function child(id) {
    location.href = URL + "/index/pid/" + id + "/group_id/" + document.getElementById("group_id").value;
}
function action(id) {
...
function check(id) {
    ...
    //
    if ( confirm("确定要通过审核吗？") ) {
    	location.href = URL + "/check/id/" + keyValue;
    }
}
...
function searchItem(item) {
    for (i = 0; i < selectSource.length; i++) if (selectSource[i].text.indexOf(item) != -1) {
...
function addItem() {
    for (i = 0; i < selectSource.length; i++) if (selectSource[i].selected) {
        selectTarget.add(new Option(selectSource[i].text, selectSource[i].value));
    }
    for (i = 0; i < selectTarget.length; i++) for (j = 0; j < selectSource.length; j++) if (selectSource[j].text == selectTarget[i].text) selectSource[j] = null;
}

function delItem() {
    for (i = 0; i < selectTarget.length; i++) if (selectTarget[i].selected) {
        selectSource.add(new Option(selectTarget[i].text, selectTarget[i].value));

    }
    for (i = 0; i < selectSource.length; i++) for (j = 0; j < selectTarget.length; j++) if (selectTarget[j].text == selectSource[i].text) selectTarget[j] = null;
}
