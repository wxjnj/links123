...
class EnglishCatquestionModel extends CommonModel {
	public function getQuestionProperty($question_id) {
		$ret = $this->alias('catquestion')
					->field('catquestion.cat_id as id, catquestion.question_id, catquestion.type, catquestion.status as status, category.cat_attr_id, category.level_one, category.level_two, category.level_thr')
					->join('left join ' . C("DB_PREFIX") . 'english_category category on catquestion.cat_id = category.cat_id')
					->where(array("catquestion.question_id" => $question_id, "level_one" => array("neq", "-1")))
					->select();
		return $ret;
	}
