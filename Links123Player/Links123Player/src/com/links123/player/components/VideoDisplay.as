...
package com.links123.player.components
{
	...
	public class VideoDisplay extends UIComponent
	{
		...
		{
			Logger.debug("是否seek:{0}",videoPlayer.canSeek);
			// check to see if we can call methods on the video player object yet
			if (!videoPlayerResponsive())
				return;
		 ...
		 * 
		 */		
		public function isCanseekto(time:Number):Boolean
		{
			if(videoPlayer.canSeek && videoPlayer.canSeekTo(time))
			{
				return true;
			}
			return false;
...
/*			if(videoPlayer.media != null){
				var lt:LoadTrait=videoPlayer.media.getTrait(MediaTraitType.LOAD) as LoadTrait;
				Logger.debug("loadtrait:{0}",lt);
				var ns:NetStream=lt["netStream"] as NetStream;
				if(ns){
					Logger.debug("netStream:{0}",ns);
					var f:Function=ns["dispose"] as Function;
					if(f){
						f.apply(ns);
					}
				
					ns.close();
					var nc:NetConnection=lt["netConnection"] as NetConnection;
					nc.close();
				}
				//lt.unload();
			}*/
			...
			if (inLoadingState1)
			{
				...
				else if (!inLoadingState3)
				{
					...
					if(videoPlayer.displayObject)
						...
						Logger.warn("cleaning up source encounters error,{0}",e);
					}
		...
		private function videoPlayer_mediaPlayerStateChangeHandlerForLoading(event:MediaPlayerStateChangeEvent):void
		{
			...
			if (event.state == MediaPlayerState.READY)
			{
				...
				try{//switching video to audio only may encounter error
					videoPlayer.play();
				}
				catch(e:Error){
					Logger.warn("trying to play from videoPlayer_mediaPlayerStateChangeHandlerForLoading failed,{0}",e);
				}
			}
		}
	...
	/*	private var _preloadingUrl:String;
		private var _preloadingMedia:MediaElement;

		pl_internal function preload(url:String):void{
			if(_preloadingUrl != null){
				cleanUpPreloadSource(true);
			}	
			_preloadingMedia=createMedia(url);
			if(_preloadingMedia != null){
				_preloadingUrl=url;
				loadMedia(_preloadingMedia,true);
			}
		}
		
		private function loadMedia(media:MediaElement,load:Boolean):void
		{
			if(load){
			...
			for each (traitType  in _preloadingMedia.traitTypes)
			{
				updateTraitListeners(traitType, load,media);
			}
			
		}
		private var preloadTimer:Timer;
		private static const  PRELOAD_TIMER_INTERVAL:Number=1000;
		private function updateTraitListeners(traitType:String, add:Boolean, media:MediaElement):void
		{
			...
			{	
				case MediaTraitType.LOAD:						
					...
					if (add)
					{
						...
						{
							preloadTimer=new Timer(PRELOAD_TIMER_INTERVAL);
							preloadTimer.addEventListener(TimerEvent.TIMER,onPreloadTimerTick);
						}
						preloadTimer.start();
						dispatchPreloadEvent(loadTrait.bytesLoaded,loadTrait.bytesTotal);
					}
					else{
						dispatchPreloadEvent(0,0);
					}
					break;
			}
		...
		protected function onPreloadTimerTick(event:TimerEvent):void
		{
			//
			if(_preloadingMedia == null || _preloadingMedia.hasTrait(MediaTraitType.LOAD) == false){
				preloadTimer.stop();
				return;
			}
			var loadTrait:LoadTrait = LoadTrait(_preloadingMedia.getTrait(MediaTraitType.LOAD));
			dispatchPreloadEvent(loadTrait.bytesLoaded,loadTrait.bytesTotal);
			trace("preload totalBytes:"+loadTrait.bytesTotal+",loadedBytes:"+loadTrait.bytesLoaded);
			if(loadTrait.bytesTotal > 0 && loadTrait.bytesLoaded>=loadTrait.bytesTotal){
				preloadTimer.stop();
			}
			
			
		}
		protected function dispatchPreloadEvent(loaded:Number,total:Number):void{
			if(preloadClient != null && preloadClient.onByteLoaded){
				var fun:Function=preloadClient.onByteLoaded as Function;
				if(fun != null){
					fun(loaded,total);
				}
			}
		...
		{
			updateTraitListeners(event.traitType, false,_preloadingMedia);	
			
		}
		
		protected function onTraitAdd(event:MediaElementEvent):void
		{
			updateTraitListeners(event.traitType, true,_preloadingMedia);	
			
		}
		
		private function createMedia(url:String):MediaElement
		{
			var urlResource:URLResource = new URLResource(url);
			var videoElement:MediaElement =new VideoElement(urlResource,new NetLoader); //mediaFactory.createMediaElement(urlResource);
		...
		private function cleanUpPreloadSource(disposing:Boolean=false):void
		{
			_preloadingUrl = null;
			
			if(_preloadingMedia){
				loadMedia(_preloadingMedia,false);
				if(disposing){
					
				}
			}
			
			_preloadingMedia=null;
			
			
		}
		
		private function processPreloadSource(value:Object):Object{
			if(_preloadingUrl == null || _preloadingMedia == null || (value is String) == false){
				 return value;
			}
			if(matchPreloadSource(value)){
				var ret:MediaElement=_preloadingMedia;
				cleanUpPreloadSource();
				return ret;
			}
			return value;
		}
		
		pl_internal var preloadClient:Object;
		private function matchPreloadSource(value:Object):Boolean
		{
			var fun:Function=null;
			if(preloadClient != null && preloadClient.matchFunc != undefined){
				fun=preloadClient.matchFunc as Function;
			}
			if(fun==null){
				fun=defaultPreloadingSourceMatchFun;
			}
			return fun(_preloadingUrl,String(value));
		}
		private function defaultPreloadingSourceMatchFun(pre:String,src:String):Boolean{
			return pre === src;
		}*/
	}
}
